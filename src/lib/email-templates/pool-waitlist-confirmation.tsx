...
const PoolWaitlistConfirmationEmail = ({
  ...
  const milesLabel =
    typeof nearestMiles === "number" ? `${Math.round(nearestMiles).toLocaleString()} miles` : null;

  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>
        You're on the {SITE_NAME} waitlist for {where}
      </Preview>
      <Body style={main}>
        <Container style={container}>
          <Heading style={h1}>You're on the list! 🎉</Heading>
          <Text style={text}>
            Thanks for joining the {SITE_NAME} waitlist for <strong>{where}</strong>. We'll email
            you the moment a host opens up a pool within driving distance.
          </Text>

          {milesLabel && (
            <Section style={infoBox}>
              <Text style={infoText}>
                The closest pool we currently have is about <strong>{milesLabel}</strong> from you.
                We're actively recruiting hosts in your area to shorten that gap.
              </Text>
            </Section>
          )}
