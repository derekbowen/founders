...
export const listSeoIssues = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ rows: SeoIssueRow[] }> => {
    ...
    const q = (supabaseAdmin as any)
      .from("content_pages")
      .select(
        "id, url_path, title, template_type, body_markdown, seo_description, updated_at, slug, status",
      )
      .eq("status", "published")
      .like("url_path", "/p/%")
...
export const listContentPages = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ rows: ContentPageRow[]; total: number }> => {
    ...
    let base: any = (supabaseAdmin as any)
      .from("content_pages")
      .select("id, url_path, title, template_type, status, body_markdown, updated_at", {
        count: "exact",
      })
      .like("url_path", "/p/%");
  ...
  .handler(async ({ context }): Promise<IndexingStats> => {
    ...
    const [
      ...
      sb
        ...
        .select("id, url_path, hit_count, last_seen_at")
        .is("resolved_at", null)
    ...
    return {
      ...
      byTemplate: Array.from(tplMap.entries())
...
export async function runSeoFix(
  pageId: string,
  mode: "full" | "meta_only" | "title_only",
): Promise<{ ok: true; newWords: number; newTitle: string } | { ok: false; error: string }> {
  ...
  const { data: page, error: pErr } = await (supabaseAdmin as any)
    .from("content_pages")
    .select(
      "id, url_path, slug, title, seo_title, seo_description, body_markdown, template_type, category",
    )
    .eq("id", pageId)
    .maybeSingle();
...
For body_markdown, return the EXISTING body unchanged.`;
    ...
    const reason =
      wordCount === 0
        ? "Page body is EMPTY — write fresh content."
        : wordCount < 500
          ? `Page body is THIN (${wordCount} words) — expand to 800-1200 words while keeping any existing facts.`
...
Length: 800-1200 words. Use ## sections and ### sub-points. Strong opening, no fluff.`;
  ...
  if (!resp.ok)
    return { ok: false, error: `AI gateway ${resp.status}: ${(await resp.text()).slice(0, 200)}` };
  ...
  .handler(
    async ({
      ...
      let q = sb
        .from("seo_fix_jobs")
        .select(
          "id, page_id, mode, status, attempts, result, error, batch_id, created_at, finished_at",
        )
        .order("created_at", { ascending: false })
        .limit(500);
