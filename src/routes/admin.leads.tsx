...
export const Route = createFileRoute("/admin/leads")({
  beforeLoad: async () => {
    ...
    if (error || !data.user)
      throw redirect({ to: "/auth", search: { redirect: "/admin/leads", mode: "signin" } });
    const { isAdmin } = await checkAdminRole();
    if (!isAdmin) throw redirect({ to: "/admin/no-access" });
  },
  head: () => ({
    meta: [{ title: "Lead Inbox — Admin" }, { name: "robots", content: "noindex,nofollow" }],
  }),
  component: LeadInbox,
});
...
function LeadInbox() {
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    try {
      setRows((await listLeads({ data: { status, limit: 200 } })).rows);
    } finally {
      setLoading(false);
    }
  }, [status]);
  ...
  return (
    <AdminLayout title="Lead Inbox">
      ...
      <div className="mt-4 text-sm text-muted-foreground">
        {loading ? "Loading…" : `${rows.length} leads`}
      </div>
          ...
          <tbody>
            {rows.map((r) => (
              ...
              >
                ...
                <td className="px-3 py-2">
                  <span
                    className={`rounded px-2 py-0.5 text-xs font-bold ${r.status === "new" ? "bg-yellow-500/20 text-yellow-700 dark:text-yellow-300" : r.status === "contacted" ? "bg-blue-500/20 text-blue-700 dark:text-blue-300" : "bg-green-500/20 text-green-700 dark:text-green-300"}`}
                  >
                    {r.status}
                  </span>
                </td>
        ...
        >
          ...
          >
            ...
            <dl className="mt-4 space-y-2 text-sm">
              ...
              {(open.city || open.state_code) && (
                <div>
                  <dt className="text-xs uppercase text-muted-foreground">Location</dt>
                  <dd>{[open.city, open.state_code].filter(Boolean).join(", ")}</dd>
                </div>
              )}
              {open.source_path && (
                <div>
                  <dt className="text-xs uppercase text-muted-foreground">Source</dt>
                  <dd className="font-mono text-xs">{open.source_path}</dd>
                </div>
              )}
              {open.message && (
                <div>
                  <dt className="text-xs uppercase text-muted-foreground">Message</dt>
                  <dd className="whitespace-pre-wrap rounded bg-muted/50 p-3">{open.message}</dd>
                </div>
              )}
            </dl>
            <div className="mt-6 flex gap-2">
