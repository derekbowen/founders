...
export const Route = createFileRoute("/admin/scrape-import")({
  beforeLoad: async () => {
    ...
    if (error || !data.user)
      throw redirect({ to: "/auth", search: { redirect: "/admin/scrape-import", mode: "signin" } });
...
function ScrapeImport() {
  ...
  const [progress, setProgress] = React.useState<{ done: number; total: number; last?: string }>({
    done: 0,
    total: 0,
  });
  const [results, setResults] = React.useState<
    Array<{ url: string; ok: boolean; error?: string; providerId?: string | null }>
  >([]);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Paste Yelp, Google Maps, BBB, Angi, Houzz, Thumbtack listing URLs. Each becomes a
            pending provider.
          </p>
        </div>
        <Link to="/admin/directory" className="text-sm text-primary hover:underline">
          ← Directory
        </Link>
      </div>
        ...
        <div className="mt-3 flex items-center gap-3">
          ...
          >
            {running
              ...
              : "Scrape & create pending providers"}
          </button>
          {progress.last && running && (
            <span className="truncate text-xs text-muted-foreground">{progress.last}</span>
          )}
        </div>
      </div>
        ...
        <ul className="mt-3 divide-y divide-border rounded-2xl border border-border bg-card">
          {jobs.map((j) => (
            <li key={j.id} className="flex flex-wrap items-center gap-3 p-3 text-xs">
              <span
                className={`rounded-full px-2 py-0.5 font-bold uppercase ${j.status === "success" ? "bg-green-500/15 text-green-700" : j.status === "failed" ? "bg-red-500/15 text-red-700" : "bg-yellow-500/15 text-yellow-700"}`}
              >
                {j.status}
              </span>
