...
import { adminGetLearnerDetail, type AdminLearnerDetail } from "@/server/learning.functions";
...
export const Route = createFileRoute("/admin/learning/$userId")({
  ...
  head: () => ({ meta: [{ title: "Learner detail — founders.click" }] }),
});
  ...
  return (
    <AdminLayout>
      ...
      <h1 className="mt-2 text-2xl font-bold tracking-tight text-foreground">
        {detail?.profile?.full_name || detail?.profile?.display_name || "Learner"}
      </h1>
                ...
                {detail.progress.map((p) => (
                  ...
                  >
                    ...
                    <div className="mt-2 flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-foreground">{p.progress_pct}%</span>
                    </div>
                    <div className="mt-2 text-xs text-muted-foreground">
                      Started {p.started_at ? new Date(p.started_at).toLocaleDateString() : "—"} ·
                      Last activity{" "}
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {c.certificate_uid} · Issued {new Date(c.completed_at).toLocaleDateString()}
                        {c.revoked_at && (
