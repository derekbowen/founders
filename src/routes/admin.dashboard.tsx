...
export const Route = createFileRoute("/admin/dashboard")({
  ...
  head: () => ({
    meta: [{ title: "Admin Dashboard — PRNM" }, { name: "robots", content: "noindex,nofollow" }],
  }),
...
function AdminDashboard() {
  ...
  const pct = stats
    ? Math.round((stats.contentPages.published / Math.max(stats.contentPages.total, 1)) * 100)
    : 0;
            ...
            <>
              ...
              <section className="mt-8">
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  ...
                  <StatCard
                    label="Published"
                    value={stats.contentPages.published.toLocaleString()}
                    tone="ok"
                  />
                  <StatCard
                    label="Pending"
                    value={stats.contentPages.pending.toLocaleString()}
                    tone={stats.contentPages.pending > 0 ? "warn" : "ok"}
                  />
                  <StatCard
                    label="Published last 24h"
                    value={stats.contentPages.last24h.toLocaleString()}
                    hint="Generation activity"
                  />
                </div>
              </section>
                  ...
                  <section className="mt-8 rounded-xl border border-red-500/40 bg-red-500/5 p-4">
                    <div className="flex items-center justify-between">
                      <h2 className="text-base font-semibold text-red-700 dark:text-red-300">
                        Critical issues — published pages
                      </h2>
                      <Link
                        to="/admin/seo-health"
                    ...
                    <ul className="mt-2 grid grid-cols-1 gap-1 text-sm sm:grid-cols-2">
                      {lines.map((l) => (
                        <li key={l.key} className="flex items-baseline gap-2">
                          <span className="inline-flex min-w-[3rem] justify-end rounded bg-red-500/15 px-2 py-0.5 text-xs font-bold text-red-700 dark:text-red-300">
                            {l.n.toLocaleString()}
                          </span>
              ...
              <section className="mt-8">
                ...
                <p className="text-xs text-muted-foreground">
                  Quality buckets count published pages only. Thin = &lt;500 words, Medium =
                  500–999, Healthy = 1000+.
                </p>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {stats.byTemplate.map((t) => {
                        ...
                        const q = stats.quality?.byTemplate.find(
                          (x) => (x.template_type || "(none)") === (t.template_type || "(none)"),
                        );
                        ...
                        return (
                          <tr key={t.template_type || "(none)"} className="border-t border-border">
                            ...
                            <td className="px-3 py-2 text-right">
                              {thin > 0 ? (
                                <span className="rounded bg-red-500/15 px-1.5 py-0.5 text-xs font-bold text-red-700 dark:text-red-300">
                                  {thin}
                                </span>
                              ) : (
                            ...
                            <td className="px-3 py-2 text-right">
                              {healthy > 0 ? (
                                <span className="rounded bg-green-500/15 px-1.5 py-0.5 text-xs font-bold text-green-700 dark:text-green-300">
                                  {healthy}
                                </span>
                              ) : (
                                <span className="text-muted-foreground">0</span>
              ...
              <section className="mt-8">
                ...
                <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                  <StatCard
                    label="Blog posts"
                    value={`${stats.blog.published}/${stats.blog.total}`}
                  ...
                  <StatCard
                    label="Help articles"
                    value={`${stats.helpArticles.published}/${stats.helpArticles.total}`}
                    hint="published / total"
                  />
                  <StatCard
                    label="Cities"
                    value={`${stats.cities.published}/${stats.cities.total}`}
                    hint="published / total"
                  />
                  <StatCard
                    label="Providers"
                    value={`${stats.providers.published}/${stats.providers.total}`}
                    hint="published / total"
                  />
                  <StatCard
                    ...
                    hint={
                      stats.listings.lastSync
                        ? `Last sync ${new Date(stats.listings.lastSync).toLocaleString()}`
                        : "No sync yet"
                    }
                  />
                </div>
              ...
              <section className="mt-8">
                ...
                <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  ...
                  <StatCard
                    label="Waitlist (7d)"
                    value={`${stats.waitlist.last7d} / ${stats.waitlist.total}`}
                    hint="last 7d / all-time"
                  />
                  <StatCard
                    label="Provider leads"
                    value={`${stats.leads.new} new / ${stats.leads.total}`}
                    tone={stats.leads.new > 0 ? "warn" : undefined}
                  />
                </div>
              </section>
              ...
              <section className="mt-8">
                ...
                <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
                  <StatCard
                    ...
                    value={stats.missing404s.unresolved.toLocaleString()}
                    hint={`${stats.missing404s.total} total logged`}
                    tone={stats.missing404s.unresolved > 10 ? "warn" : undefined}
                  />
                </div>
              </section>
              ...
              <section className="mt-8">
                ...
                <ul className="mt-3 divide-y divide-border rounded-xl border border-border">
                  {stats.recentlyPublished.map((r) => (
                    ...
                    >
                      <Link to={r.url_path} className="truncate font-medium hover:underline">
                        {r.title || r.url_path}
                      </Link>
                      ...
                      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                        {g.items.map((it) => (
                          <Link
                            key={it.to}
                            to={it.to}
                            className="group flex items-center gap-2.5 rounded-lg border border-border bg-background p-3 text-sm font-medium hover:border-primary hover:bg-primary/5"
                          >
