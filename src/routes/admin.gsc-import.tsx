...
export const Route = createFileRoute("/admin/gsc-import")({
  beforeLoad: async () => {
    ...
    if (error || !data.user)
      throw redirect({ to: "/auth", search: { redirect: "/admin/gsc-import", mode: "signin" } });
    const { isAdmin } = await checkAdminRole();
    if (!isAdmin) throw redirect({ to: "/admin/no-access" });
  },
  head: () => ({
    meta: [{ title: "GSC Import — Admin" }, { name: "robots", content: "noindex,nofollow" }],
  }),
  component: GscImport,
});
  ...
  async function handleFiles(files: FileList | null) {
    ...
    try {
      ...
      if (/\.zip$/i.test(file.name) || file.type === "application/zip") {
        ...
        const pick =
          names.find((n) => /pages\.csv$/i.test(n)) || names.find((n) => /\.csv$/i.test(n));
        if (!pick) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Export from GSC: Performance → Pages → filter URL contains{" "}
            <code className="rounded bg-secondary px-1">/providers/</code> → Export → CSV. Paste
            below.
          </p>
        </div>
      ...
      <div className="mt-6 rounded-2xl border border-border bg-card p-5">
        <label className="flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-border p-8 text-center hover:bg-secondary/40 cursor-pointer">
          ...
          <span className="text-xs text-muted-foreground">
            Drag a Google Search Console export here, or click to browse. ZIPs are auto-extracted
            (Pages.csv).
          </span>
      ...
      {parsed.length > 0 && (
        <section className="mt-6">
          ...
          <div className="mt-3 overflow-x-auto rounded-2xl border border-border bg-card">
            ...
            {parsed.length > 50 && (
              <p className="p-2 text-xs text-muted-foreground">…and {parsed.length - 50} more</p>
            )}
          </div>
        </section>
      )}
