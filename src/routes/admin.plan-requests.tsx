  ...
  return (
    <AdminLayout>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <Link to="/admin/dashboard" className="text-sm text-primary underline">
            ← Admin
          </Link>
        ...
        {pending.map((r) => (
          <li key={r.id} className="rounded-2xl border bg-card p-4 shadow-sm">
            ...
            <div className="mt-2 grid gap-1 text-sm sm:grid-cols-2">
              <div>
                <strong>Requester:</strong> {r.requester_name} &lt;{r.requester_email}&gt;
                {r.requester_phone ? ` · ${r.requester_phone}` : ""}
              </div>
              <div>
                <strong>Payment:</strong> {r.payment_method || "—"} ·{" "}
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <button
                ...
                className="rounded-full border px-4 py-1.5 text-sm font-semibold disabled:opacity-50"
              >
                Reject
              </button>
              <button
                ...
                className="ml-auto rounded-full px-4 py-1.5 text-sm font-semibold text-rose-700 disabled:opacity-50"
              >
                Delete
              </button>
            </div>
      ...
      <ul className="mt-3 space-y-2">
        {reviewed.map((r) => (
          ...
          >
            ...
            <span
              className={`rounded-full px-2 py-0.5 text-xs font-semibold ${r.status === "approved" ? "bg-emerald-100 text-emerald-800" : "bg-rose-100 text-rose-800"}`}
            >
              {r.status}
            </span>
            ...
            <button onClick={() => act(r.id, "delete")} className="text-xs text-rose-700 underline">
              delete
            </button>
          </li>
        ))}
