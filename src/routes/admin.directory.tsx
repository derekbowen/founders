...
import {
  adminListPendingProviders,
  adminUpdateProvider,
  adminGenerateProviderContent,
  adminListProvidersMissingAI,
} from "@/server/directory.functions";
...
export const Route = createFileRoute("/admin/directory")({
  beforeLoad: async () => {
    ...
    if (error || !data.user)
      throw redirect({ to: "/auth", search: { redirect: "/admin/directory", mode: "signin" } });
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      const r = await adminListPendingProviders({
        data: { page, pageSize, status: filter, search },
      });
      setRows(r.providers);
      setTotal(r.total);
    } finally {
      ...
      try {
        for (const p of targets) {
          ...
          try {
            ...
            setBulkResults((prev) => [
              ...prev,
              { id: p.id, name: p.name, ok: true, ms: Date.now() - t0 },
            ]);
          } catch (e: any) {
            setBulkResults((prev) => [
              ...prev,
  ...
  return (
    <AdminLayout>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        ...
        <Link
          to="/admin/scrape-import"
          className="rounded-full border border-border bg-card px-3 py-1 text-xs font-semibold hover:bg-secondary"
        >
          + Scrape URL
        </Link>
        <Link
        ...
        <button
          onClick={load}
          className="ml-auto rounded-full bg-card border border-border px-3 py-1 text-xs"
        >
          Refresh
        </button>
      </div>
      ...
      {(bulkRunning || bulkResults.length > 0) && (
        <div className="mt-4 rounded-lg border border-border bg-card p-4">
          ...
          {bulkCurrent && (
            <p className="mt-2 text-xs text-muted-foreground">Current: {bulkCurrent}</p>
          )}
          {bulkResults.length > 0 && (
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {[p.primary_category, [p.city, p.state_code].filter(Boolean).join(", ")]
                      .filter(Boolean)
                      .join(" · ")}
                  </p>
                  <p className="mt-2 max-w-2xl text-sm text-muted-foreground line-clamp-3">
                    {p.description}
                  </p>
                  ...
                  <div className="mt-2 flex flex-wrap gap-3 text-xs">
                    <TimestampPill
                      label="Paid until"
                      value={p.listing_paid_until}
                      activeClass="text-green-700"
                    />
                    <TimestampPill
                      label="Featured until"
                      value={p.featured_until}
                    ...
                    {(p.gsc_impressions || p.gsc_clicks) && (
                      <span
                        ...
                        title={
                          p.gsc_updated_at
                            ? `Updated ${new Date(p.gsc_updated_at).toLocaleString()}`
                            : undefined
                        }
                      >
                        GSC: {p.gsc_impressions ?? 0} impr · {p.gsc_clicks ?? 0} clk
                        {p.gsc_position ? ` · pos ${Number(p.gsc_position).toFixed(1)}` : ""}
                      </span>
                    )}
                    {p.ai_content_generated_at && (
                      <span className="text-muted-foreground">AI ✓</span>
                    )}
                    {p.source_type && (
                      <span className="text-muted-foreground">src: {p.source_type}</span>
                    )}
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  ...
                  {p.submission_status === "approved" && (
                    <>
                      ...
                      <Btn
                        ...
                        busy={busy === p.id + "mark_paid"}
                        tone="ok"
                      >
                        Mark paid ($5/yr)
                      </Btn>
                      {p.listing_paid_until && (
                        <Btn
                          onClick={() => act(p.id, "mark_unpaid")}
                          busy={busy === p.id + "mark_unpaid"}
                        >
                          Mark unpaid
                        </Btn>
                      )}
                  ...
                  <Btn
                    ...
                    tone="primary"
                  >
                    {p.ai_content_generated_at ? "↻ Regen AI" : "✨ Gen AI content"}
                  </Btn>
                  <Btn
                    onClick={() => {
...
function TimestampPill({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 ${isActive ? activeClass : "text-muted-foreground"}`}
    >
      <span title={iso || "—"}>
        {label}: {fmtDate(value)}
...
function Btn({
  ...
  return (
    <button
      onClick={onClick}
      disabled={busy}
      className={`rounded-full px-3 py-1.5 text-xs font-semibold disabled:opacity-50 ${cls}`}
    >
      {busy ? "…" : children}
    </button>
  );
