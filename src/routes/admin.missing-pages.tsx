...
import { list404s, resolve404, type Content404Row } from "@/server/content-404-log.functions";
  ...
  return (
    <AdminLayout>
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Missing /p/* pages</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {rows.length} unique URL{rows.length === 1 ? "" : "s"} · {totalHits} total 404 hit
            {totalHits === 1 ? "" : "s"}
          </p>
        </div>
