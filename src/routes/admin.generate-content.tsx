  ...
  return (
    <AdminLayout>
      ...
      <p className="mt-2 text-muted-foreground">
        Pulls pending rows from <code>content_plan</code> (3,286 prioritized pages), generates each
        one with Gemini using its own H1, keywords, and uniqueness angle, validates internal links +
        FAQ, then inserts into <code>content_pages</code>. No SQL copy-paste, no doorway pages.
      </p>
        ...
        <div className="rounded-md border border-border bg-muted/30 p-4 text-sm">
          ...
          {preflight.details && (
            <ul className="mt-3 space-y-1 text-xs text-muted-foreground">
              <li>
                Edge function:{" "}
                <span className="font-mono">{preflight.details.edgeFunction ?? "unreachable"}</span>
              </li>
              <li>
                Admin auth:{" "}
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <span
                    className={`rounded px-2 py-0.5 text-xs font-mono ${
                      e.ok ? "bg-green-500/10 text-green-700" : "bg-destructive/10 text-destructive"
                    }`}
                  >
                    {e.ok ? "OK" : "FAIL"}
                  </span>
              ...
              {validationErrors.length > 0 && (
                <details className="mt-3" open>
                  <summary className="cursor-pointer text-sm font-medium text-destructive">
                    {validationErrors.length} validation note(s) — click to see per-slug failure
                    reason
                  </summary>
