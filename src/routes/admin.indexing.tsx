...
export const Route = createFileRoute("/admin/indexing")({
  beforeLoad: async () => {
    ...
    if (error || !data.user)
      throw redirect({ to: "/auth", search: { redirect: "/admin/indexing", mode: "signin" } });
  ...
  return (
    <AdminLayout title="Sitemap & Indexing">
      ...
      <p className="text-sm text-muted-foreground">
        Sitemap inventory, recent indexing activity, and unresolved 404s.
      </p>
        ...
        <>
          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            <div className="rounded-xl border border-border p-4">
              <div className="text-xs uppercase text-muted-foreground">Published /p/*</div>
              <div className="mt-1 text-2xl font-bold">{s.totalPublished.toLocaleString()}</div>
            </div>
            <div className="rounded-xl border border-border p-4">
              <div className="text-xs uppercase text-muted-foreground">Published last 24h</div>
              <div className="mt-1 text-2xl font-bold">{s.recentlyPublished.toLocaleString()}</div>
            </div>
            <div className="rounded-xl border border-border p-4">
              <div className="text-xs uppercase text-muted-foreground">Unresolved 404s</div>
              <div className="mt-1 text-2xl font-bold text-red-600">
                {s.unresolved404s.toLocaleString()}
              </div>
            </div>
            <div className="rounded-xl border border-border p-4">
              <div className="text-xs uppercase text-muted-foreground">Sitemap files</div>
              <div className="mt-1 text-2xl font-bold">{SITEMAPS.length}</div>
            </div>
          </div>
