    ...
    <AdminLayout title="Site Footer">
      <div className="flex items-center justify-between gap-2 pb-6">
        <p className="text-sm text-muted-foreground">
          Edit company links, contact info, social icons, and popular markets shown in the global
          footer.
        </p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4">
            <Field
              label="Phone label"
              value={data.contact_phone_label ?? ""}
              onChange={(v) => update("contact_phone_label", v || null)}
              placeholder="Call us 888-940-4247"
            />
            <Field
              label="Phone link (tel:)"
              value={data.contact_phone ?? ""}
              onChange={(v) => update("contact_phone", v || null)}
              placeholder="tel:18889404247"
            />
            <Field
              label="Hours"
              value={data.contact_phone_hours ?? ""}
              onChange={(v) => update("contact_phone_hours", v || null)}
              placeholder="10am - 5pm PST"
            />
            <Field
              label="Support email"
              value={data.contact_email ?? ""}
              onChange={(v) => update("contact_email", v || null)}
            ...
            {data.socials.map((s, i) => (
              <div key={i} className="grid grid-cols-[1fr_1fr_2fr_auto] gap-2">
                <select
                  ...
                  value={s.icon}
                  onChange={(e) =>
                    updateArrayItem(setData, "socials", i, { ...s, icon: e.target.value })
                  }
                >
                ...
                <Input
                  value={s.label}
                  onChange={(e) =>
                    updateArrayItem(setData, "socials", i, { ...s, label: e.target.value })
                  }
                  placeholder="Label"
                />
                <Input
                  value={s.href}
                  onChange={(e) =>
                    updateArrayItem(setData, "socials", i, { ...s, href: e.target.value })
                  }
                  placeholder="https://…"
                />
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => removeArrayItem(setData, "socials", i)}
                >
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {data.popular_markets.map((mkt, i) => (
              <div key={i} className="grid grid-cols-[1fr_1fr_auto] gap-2">
                <Input
                  value={mkt.name}
                  onChange={(e) =>
                    updateArrayItem(setData, "popular_markets", i, { ...mkt, name: e.target.value })
                  }
                  placeholder="City, ST"
                />
                <Input
                  value={mkt.slug}
                  onChange={(e) =>
                    updateArrayItem(setData, "popular_markets", i, { ...mkt, slug: e.target.value })
                  }
                  placeholder="city-st"
                />
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => removeArrayItem(setData, "popular_markets", i)}
                >
                  <Trash2 className="h-4 w-4" />
                </Button>
              </div>
            ...
            <Button
              ...
              size="sm"
              onClick={() =>
                addArrayItem(setData, "popular_markets", { name: "", slug: "" } as FooterMarket)
              }
            >
      ...
      <CardContent className="space-y-3">
        {items.map((it, i) => (
          <div key={i} className="grid grid-cols-[1fr_2fr_auto] gap-2">
            <Input
              value={it.label}
              onChange={(e) => updateArrayItem(setData, field, i, { ...it, label: e.target.value })}
              placeholder="Label"
            />
            <Input
              value={it.href}
              onChange={(e) => updateArrayItem(setData, field, i, { ...it, href: e.target.value })}
              placeholder="/path or https://…"
            />
            <Button variant="ghost" size="icon" onClick={() => removeArrayItem(setData, field, i)}>
              <Trash2 className="h-4 w-4" />
            </Button>
          </div>
        ))}
        <Button
          variant="outline"
          size="sm"
          onClick={() => addArrayItem(setData, field, { label: "", href: "" } as FooterLink)}
        >
