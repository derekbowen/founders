...
export const Route = createFileRoute("/admin/seo-health")({
  beforeLoad: async () => {
    ...
    if (error || !data.user)
      throw redirect({ to: "/auth", search: { redirect: "/admin/seo-health", mode: "signin" } });
    const { isAdmin } = await checkAdminRole();
    if (!isAdmin) throw redirect({ to: "/admin/no-access" });
  },
  head: () => ({
    meta: [{ title: "SEO Health — Admin" }, { name: "robots", content: "noindex,nofollow" }],
  }),
  component: SeoHealth,
});
...
const KINDS = [
  ...
  { id: "empty", label: "Empty body", fixMode: "full" as const, fixLabel: "Generate body" },
  {
    ...
    fixMode: "meta_only" as const,
    fixLabel: "Generate meta",
  },
  {
    id: "title_is_slug",
    label: "Title is just slug",
    fixMode: "title_only" as const,
...
function SeoHealth() {
  ...
  const load = React.useCallback(async () => {
    setLoading(true);
    try {
      const r = await listSeoIssues({ data: { kind: kindId, limit: 200 } });
      setRows(r.rows);
      setSelected(new Set());
    } finally {
      setLoading(false);
  ...
  React.useEffect(() => {
    ...
    const tick = async () => {
      ...
      const stillRunning = Object.values(jobs).some(
  ...
  async function enqueueBatch(targets: SeoIssueRow[]) {
    ...
    const res = await enqueueSeoFixJobs({
      data: { pageIds: targets.map((t) => t.id), mode: kind.fixMode },
    });
    if (res.ok) {
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Drill into published /p/* pages with quality issues, then fix them with AI.
          </p>
        </div>
        <Link
          to="/admin/generate-content"
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 rounded-lg border border-border bg-card p-3">
        ...
        <div className="ml-auto flex flex-wrap gap-2">
          ...
          {batchId && (summary.queued > 0 || summary.processing > 0) && (
            <button
              onClick={cancelBatch}
              className="rounded-md border border-red-500 px-3 py-1.5 text-sm font-semibold text-red-600"
            >
              Cancel queue
            </button>
          )}
      ...
      {summary.queued + summary.processing + summary.done + summary.failed > 0 && (
        <div className="mt-3 rounded-lg border border-border bg-card p-3">
          <div className="flex flex-wrap items-center gap-3 text-xs">
            ...
            <span className="rounded bg-blue-500/20 px-2 py-0.5 text-blue-700 dark:text-blue-300">
              ⚙ Processing {summary.processing}
            </span>
            <span className="rounded bg-green-500/20 px-2 py-0.5 text-green-700 dark:text-green-300">
              ✓ Done {summary.done}
            </span>
            {summary.failed > 0 && (
              <span className="rounded bg-red-500/20 px-2 py-0.5 text-red-700 dark:text-red-300">
                ✗ Failed {summary.failed}
              </span>
            )}
            <span className="ml-auto text-muted-foreground">
              Worker runs every minute. UI auto-refreshes.
            </span>
          </div>
          {(() => {
              ...
              return (
                <tr key={r.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2 text-xs">
                    {job ? (
                      job.status === "queued" ? (
                        <span className="rounded bg-muted px-1.5 py-0.5 font-bold">⏳ Queued</span>
                      ) : job.status === "processing" ? (
                        <span className="rounded bg-blue-500/20 px-1.5 py-0.5 font-bold text-blue-700 dark:text-blue-300">
                          ⚙ Processing
                        </span>
                      ) : job.status === "done" ? (
                        <span className="rounded bg-green-500/20 px-1.5 py-0.5 font-bold text-green-700 dark:text-green-300">
                          ✓ {job.result?.newWords ? `${job.result.newWords}w` : "ok"}
                        </span>
                      ) : job.status === "failed" ? (
                        <span
                          className="rounded bg-red-500/20 px-1.5 py-0.5 font-bold text-red-700 dark:text-red-300"
                          title={job.error || ""}
                        >
