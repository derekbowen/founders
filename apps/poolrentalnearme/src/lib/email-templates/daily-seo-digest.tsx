      ...
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={statRow}>
            <Stat
              label="New competitor pages"
              value={String(totalNewCompetitor)}
              tone={totalNewCompetitor > 0 ? "warn" : "ok"}
            />
            <Stat
              label="Critical audit flags"
              value={String(totalCriticalAudits)}
              tone={totalCriticalAudits > 0 ? "danger" : "ok"}
            />
            <Stat
              label="Rank drops"
              value={String(rankDrops.length)}
              tone={rankDrops.length > 0 ? "warn" : "ok"}
            />
          </Section>
          ...
          {nothing && (
            <Section style={card}>
              <Text style={text}>
                ✅ Nothing critical in the last 24 hours. All tracked competitors quiet, no audit
                scores below 60, no rank drops.
              </Text>
            </Section>
          )}
            ...
            <>
              ...
              <Section style={card}>
                ...
                {totalNewCompetitor > newCompetitorPages.length && (
                  <Text style={moreText}>
                    …and {totalNewCompetitor - newCompetitorPages.length} more
                  </Text>
                )}
              </Section>
            </>
          ...
          {criticalAudits.length > 0 && (
            <>
              <Heading as="h2" style={h2}>
                Critical AI audit flags ({totalCriticalAudits})
              </Heading>
              <Section style={card}>
                ...
                {totalCriticalAudits > criticalAudits.length && (
                  <Text style={moreText}>
                    …and {totalCriticalAudits - criticalAudits.length} more
                  </Text>
                )}
              </Section>
            </>
          ...
          {rankDrops.length > 0 && (
            <>
              <Heading as="h2" style={h2}>
                Significant rank drops ({rankDrops.length})
              </Heading>
              <Section style={card}>
                {rankDrops.map((r) => (
                  <Text key={r.keyword} style={rowText}>
                    <strong>{r.keyword}</strong>: {r.previous_position ?? "—"} →{" "}
                    <span style={{ color: "#dc2626", fontWeight: 600 }}>
                      {r.current_position ?? "lost"}
                    </span>
                  </Text>
                ))}
              </Section>
          ...
          {hostLeads.length > 0 && (
            <>
              <Heading as="h2" style={h2}>
                🎯 New host leads ({totalHostLeads})
              </Heading>
              <Section style={card}>
                <Text style={{ ...text, fontSize: "12px", marginBottom: "12px" }}>
                  Potential identity matches for competitor listings (above-board sources only —
                  public business pages, Yelp, host-published contact info).
                </Text>
                  ...
                  <Text key={i} style={rowText}>
                    ...
                    {h.candidate_email && (
                      <>
                        {" "}
                        —{" "}
                        <Link href={`mailto:${h.candidate_email}`} style={linkStyle}>
                          {h.candidate_email}
                        </Link>
                      </>
                    )}
                    ...
                    <br />
                    <Link href={h.competitor_url} style={{ ...linkStyle, fontSize: "11px" }}>
                      view listing
                    </Link>
                  </Text>
                ))}
          ...
          <Text style={footer}>
            ...
            <Link href="https://www.poolrentalnearme.com/admin/dashboard" style={linkStyle}>
              admin dashboard
            </Link>
            .
          </Text>
...
export const template = {
  ...
  subject: (data: Record<string, any>) => {
    const n =
      (data.totalNewCompetitor || 0) +
      (data.totalCriticalAudits || 0) +
      ((data.rankDrops || []).length || 0);
    return n > 0
  ...
  previewData: {
    ...
    newCompetitorPages: [
      {
        url: "https://swimply.com/pools/example",
        domain: "swimply.com",
        title: "Pool Rental in Austin",
      },
    ],
    criticalAudits: [
