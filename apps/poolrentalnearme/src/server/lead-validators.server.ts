 ...
 */
export function validateUSPhone(
  raw: string,
  ...
  if (!VALID_AREA_CODES.has(areaCode))
    return { ok: false, reason: `invalid area code ${areaCode}` };
  if (central.startsWith("0") || central.startsWith("1"))
    return { ok: false, reason: "invalid central office code" };
  ...
  if (sourceContext) {
    ...
    if (idx >= 0) {
      ...
      if (/(sku|product|item|id|asin|gtin|upc|ref|pid|prd)[=:_\-/]?\s*$/i.test(before))
        return { ok: false, reason: "appears as product identifier" };
      if (/^[\w-]*\//.test(after) && /\//.test(before))
        return { ok: false, reason: "appears mid-path" };
      if (/\.(jpg|jpeg|png|gif|webp|pdf|html?)\b/i.test(after.slice(0, 8)))
        return { ok: false, reason: "appears in filename" };
      if (window.includes("href=") && (before.endsWith("/") || before.endsWith("=")))
        return { ok: false, reason: "appears in href" };
      if (/amazonaws\.com|cloudfront|s3\./i.test(window))
        return { ok: false, reason: "appears in CDN/bucket URL" };
    }
  }
...
export function validateEmail(
...
): { ok: boolean; reason?: string; isRole?: boolean } {
  ...
  const isRole = ROLE_EMAIL_PREFIXES.some(
    (p) => local === p || local.startsWith(`${p}+`) || local.startsWith(`${p}.`),
  );
  if (isRole) {
    if (opts?.firstName && local.includes(opts.firstName.toLowerCase()))
      return { ok: true, isRole: true };
...
export function scoreCandidate(input: {
  ...
  if (input.candidate_phone) {
    ...
    if (
      ...
      validateEmail(input.candidate_email, { firstName: input.host_first_name }).ok
    ) {
      checks.push({
        label: "Phone format valid",
        passed: false,
        points: 15,
        max: 30,
        detail: "no phone, but email valid (half credit)",
      });
    } else {
      checks.push({
        label: "Phone format valid",
  ...
  const isPerson =
    looksLikeFirstName(nameToCheck) || (!!nameToCheck && !containsBusinessEntity(nameToCheck));
  ...
  checks.push({
    ...
    detail: !nameToCheck
      ? "no name"
      : isBusiness
        ? `business entity detected: "${nameToCheck}"`
        : `"${nameToCheck}"`,
  });
  ...
  const text = (
    (input.result_text || "") +
    " " +
    (input.candidate_website || "") +
    " " +
    (input.candidate_source_url || "")
  ).toLowerCase();
  let geoPts = 0;
  ...
  if (cityMatch && stateMatch) {
    geoPts = 20;
    geoDetail = `city + state match (${input.host_city}, ${input.host_state})`;
  } else if (cityMatch) {
    geoPts = 15;
    geoDetail = `city match (${input.host_city})`;
  } else if (stateMatch) {
    geoPts = 8;
    geoDetail = `state match (${input.host_state})`;
  }
  checks.push({
    label: "Geographic match to listing",
    passed: geoPts >= 15,
    points: geoPts,
    max: 20,
    detail: geoDetail,
...
export function runSelfTests() {
  const cases = [
    ...
    },
    {
      ...
      input: () => ({
        ok: !containsBusinessEntity("Warehousing Foundation d.o.o."),
    ...
    },
    {
      ...
      input: () => ({
        ok: !isStoplistedUrl("https://phonenumberlookupbcqx.s3.amazonaws.com/foo.html"),
        reason: "s3 bucket",
      }),
      expectReject: true,
    },
    {
      ...
      input: () => ({
        ok: !isStoplistedUrl("https://www.jansport.com/products/abc"),
        reason: "jansport",
      }),
      expectReject: true,
    },
    {
      ...
      input: () => ({
        ok: !containsBusinessEntity("Auto Repair Consulate GmbH"),
        reason: "business entity",
      }),
      expectReject: true,
    },
    ...
    },
    {
      ...
      input: () => ({
        ok: !isStoplistedUrl("https://example.de/foo", { listingIsUS: true }),
        reason: "foreign tld",
      }),
      expectReject: true,
    },
    {
      ...
      input: () => ({
        ok: isAllowlistedSource("https://www.whitepages.com/name/Dee-Smith"),
        reason: "allowlist",
      }),
      expectReject: false,
    },
