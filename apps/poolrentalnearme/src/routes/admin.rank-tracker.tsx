...
export const Route = createFileRoute("/admin/rank-tracker")({
  beforeLoad: async () => {
    ...
    if (error || !data.user)
      throw redirect({ to: "/auth", search: { redirect: "/admin/rank-tracker", mode: "signin" } });
    const { isAdmin } = await checkAdminRole();
    if (!isAdmin) throw redirect({ to: "/admin/no-access" });
  },
  head: () => ({
    meta: [{ title: "Rank Tracker — Admin" }, { name: "robots", content: "noindex,nofollow" }],
  }),
...
function RankTracker() {
  ...
  async function add() {
    if (!kw.trim()) return;
    const r: any = await addTrackedKeyword({
      data: { keyword: kw.trim(), target_url_path: target.trim() || undefined },
    });
    if (r.ok) {
      setKw("");
      setTarget("");
    ...
    <AdminLayout title="Rank Tracker">
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Daily Google position checks for your priority keywords. Win/loss arrows show movement
      ...
      <div className="rounded-2xl border border-border bg-card p-4">
        ...
        <div className="mt-2 grid gap-2 sm:grid-cols-3">
          <input
            ...
            placeholder="pool rental austin"
            className="rounded-lg border border-border bg-background px-3 py-2 text-sm sm:col-span-2"
          />
          <input
            value={target}
            onChange={(e) => setTarget(e.target.value)}
            placeholder="/p/austin-tx (optional)"
            className="rounded-lg border border-border bg-background px-3 py-2 text-sm font-mono"
          />
          <button
            onClick={add}
            className="inline-flex items-center justify-center gap-2 rounded-full bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground sm:col-span-3"
          >
              ...
              <tr key={r.id}>
                ...
                <td className="px-3 py-2 text-center">
                  <div className="inline-flex items-center gap-1.5">
                    ...
                    <span
                      className={`font-bold ${r.last_position == null ? "text-muted-foreground" : r.last_position <= 3 ? "text-emerald-600" : r.last_position <= 10 ? "text-foreground" : "text-muted-foreground"}`}
                    >
                      {r.last_position ?? "—"}
                    </span>
                  </div>
