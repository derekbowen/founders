...
export const Route = createFileRoute("/admin/content-pages")({
  beforeLoad: async () => {
    ...
    if (error || !data.user)
      throw redirect({ to: "/auth", search: { redirect: "/admin/content-pages", mode: "signin" } });
    const { isAdmin } = await checkAdminRole();
    if (!isAdmin) throw redirect({ to: "/admin/no-access" });
  },
  head: () => ({
    meta: [{ title: "Bulk page editor — Admin" }, { name: "robots", content: "noindex,nofollow" }],
  }),
      ...
      {selected.size > 0 && (
        <div className="mt-4 flex items-center gap-2 rounded-lg border border-primary/40 bg-primary/5 p-3">
          ...
          <button
            ...
            className="ml-auto rounded bg-green-600 px-3 py-1 text-xs font-semibold text-white disabled:opacity-50"
          >
            Publish
          </button>
          <button
            ...
            className="rounded bg-yellow-600 px-3 py-1 text-xs font-semibold text-white disabled:opacity-50"
          >
            Unpublish
          </button>
          <button
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-border">
                ...
                <td className="px-3 py-2">
                  <span
                    className={`rounded px-1.5 py-0.5 text-xs ${r.status === "published" ? "bg-green-500/20 text-green-700 dark:text-green-300" : "bg-yellow-500/20 text-yellow-700 dark:text-yellow-300"}`}
                  >
                    {r.status}
                  </span>
                </td>
