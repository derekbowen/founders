      ...
      <main className="mx-auto flex w-full max-w-xl flex-1 flex-col justify-center px-4 py-12">
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            You're signed in, but this account doesn't have admin access yet. Send the details below
            to an existing admin to be granted access.
          </p>
          ...
          {id && (
            <div className="mt-6 space-y-3 rounded-xl border border-border bg-background p-4 text-sm">
              ...
              <Row
                label="Email"
                value={id.email || "—"}
                onCopy={id.email ? () => copy(id.email!) : undefined}
              />
              <Row
                label="User ID"
                value={id.userId || "—"}
                mono
                onCopy={id.userId ? () => copy(id.userId!) : undefined}
              />
            </div>
          )}
  ...
  return (
    <div className="flex items-start justify-between gap-4">
      ...
      {onCopy && (
        <button
          onClick={onCopy}
          className="shrink-0 rounded border border-border bg-card px-2 py-1 text-xs font-medium hover:bg-muted"
        >
          Copy
        </button>
      )}
