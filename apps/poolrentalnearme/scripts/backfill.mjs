...
const STATE_LONG = Object.fromEntries(
  Object.entries(STATE_NAMES).map(([c, n]) => [
  ...
  if (m) {
    ...
    if (code)
      return {
        ...
        city,
        stateCode: code,
        stateName: STATE_NAMES[code.toLowerCase()] || code,
      };
  }
  const a = slug.match(/^([a-z-]+)-pool-host-advocacy-guide$/i);
  if (a) {
    const code = STATE_LONG[a[1].toLowerCase()];
    if (code)
      return {
        kind: "state_advocacy",
        stateCode: code,
        stateName: STATE_NAMES[code.toLowerCase()],
      };
  }
  if (tt === "public_pool" || /public-pool/.test(slug))
    return { kind: "public_pool", topic: tc(slug.replace(/-/g, " ")) };
...
function userPrompt(row) {
  ...
  if (cls.kind === "host_city")
    return `Host-recruitment landing page for ${cls.city}, ${cls.stateCode}. 800-1100 words. Title: "Become a Pool Host in ${cls.city}, ${cls.stateCode}". Sections (## headings, paraphrase): local opportunity, what pools rent best in ${cls.city}, peak seasons for ${cls.stateName}, pricing tips, why PRNM (10% fee, insurance) link /p/earnings-calculator and /p/hosting, 3-5 nearby cities, FAQ (5 Q's). End with CTA linking /l/draft/00000000-0000-0000-0000-000000000000/new/details. seo_title ≤60: "Become a Pool Host in ${cls.city}, ${cls.stateCode} | PRNM". seo_description ≤155 chars.`;
  if (cls.kind === "state_advocacy")
    return `Pool Host Advocacy & Legality guide for ${cls.stateName} (${cls.stateCode}). 1000-1400 words. Cautious language ("typically", "in most jurisdictions"). Sections: legality status, permits/licenses, HOA defense (link /p/hoa-pool-rental-defense-kit), insurance ($2M included), tax (Sched E vs C, lodging), compliant ops (link /p/sign-a-waiver), 5-Q FAQ. End CTA linking /l/draft/00000000-0000-0000-0000-000000000000/new/details.`;
  if (cls.kind === "public_pool")
    return `Article about: "${cls.topic}" — likely a public pool / community pool topic for renters. 700-1000 words. Practical info, then pivot to renting a private pool nearby on PRNM (link /s and /p/how-it-works). 3+ internal links.`;
  if (cls.kind === "event_guide")
    return `Event/city guide article: "${cls.topic}". 700-1000 words. Useful local info for someone planning the event/visit, mention renting a pool venue on PRNM (link /s?address= with the city if obvious). 3+ internal links.`;
...
const TOOL = {
  ...
  function: {
    name: "write_page",
    description: "Return generated page content.",
    parameters: {
      type: "object",
      properties: {
        title: { type: "string" },
...
async function callAI(row) {
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    if (!p.body_markdown || p.body_markdown.length < 400)
      throw new Error(`short body ${p.body_markdown?.length}`);
...
async function processOne(row) {
  try {
    ...
    return {
      ok: true,
      url: row.url_path,
      words: g.body_markdown.split(/\s+/).length,
      model: g._model,
    };
  } catch (e) {
...
async function main() {
  let q = sb
    .from("content_pages")
    .select(
      "id, slug, url_path, template_type, title, seo_title, seo_description, body_markdown, status, priority",
    )
    .like("url_path", "/p/%")
  ...
  const candidates = (rows || [])
    .filter((r) => !r.body_markdown || r.body_markdown.length < 200)
    .slice(0, BATCH);
  console.log(
    `found ${candidates.length} candidates (model=${MODEL}, conc=${CONC}${DRY ? ", DRY RUN" : ""})`,
  );

  if (DRY) {
    candidates.forEach((r) =>
      console.log(
        `  - ${r.url_path}  [${r.template_type || "-"}]  body=${r.body_markdown?.length || 0}`,
      ),
    );
    return;
  }
  ...
  async function worker() {
    while (i < candidates.length) {
      ...
      if (res.ok) {
        ok++;
        console.log(`✓ ${res.url} (${res.words}w, ${res.model})`);
      } else {
        fail++;
        console.log(`✗ ${res.url} — ${res.error}`);
      }
      // hard stop on credits exhaustion
      if (!res.ok && /402/.test(res.error)) {
        console.error("Stopping: out of credits.");
        process.exit(2);
      }
    }
  }
